...
-- ... (continue here)
-- "a" :| take (2 - 1) (replicate (5 - 1) "a")
-- "a" :| take (2 - 1) (replicate 4 "a")
-- "a" :| take (2 - 1) (a :| replicate (4 - 1) "a")
-- "a" :| take 1 (a :| replicate (4 - 1) "a")
-- "a" :| "a" :| take (1 - 1) (replicate (4 - 1) "a")
-- "a" :| "a" :| take (1 - 1) (replicate 3 "a")
-- "a" :| "a" :| take (1 - 1) ("a" :| replicate (3 - 1) "a")
-- "a" :| "a" :| take 0 ("a" :| replicate (3 - 1) "a")
-- "a" :| "a" :| EndOfList
...
charsFromLines lines =
  step [] lines
  where
    step :: String -> MyList String -> MyList Char
    step (c : cs) remaining = c :| step cs remaining
    step []       (l :| ls) = step l ls
    step []       EndOfList = EndOfList
...
foldr folder state (x :| xs) =
  x `folder` foldr folder state xs
foldr folder state EndOfList =
  state
...
mapUsingFoldr fn list =
  foldr (\a s -> fn a :| s) EndOfList list


-- Do you think your mapUsingFoldr can be used on infinite lists
-- or very long lists without materialising the whole list?
-- Answer: Yes, because it arranges the recursion to occur on the
-- "rest of the list" side of the :| constructor when that is used
...
filterUsingFoldr predicate list =
  foldr (\a s -> if predicate a then a :| s else s) EndOfList list
...
charsFromLinesWithFoldr lines =
  foldr (\line s -> DF.foldr (\c s' -> c :| s') s line) EndOfList lines
